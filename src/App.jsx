...
import { LogIn, Plus, X, MapPin, Save, LogOut, Search, Navigation, Bell, ExternalLink, Eye, EyeOff, AlertCircle, ChevronDown, ChevronUp } from 'lucide-react';
import 'leaflet/dist/leaflet.css';
import { locationsData } from './data/locations';
import L from 'leaflet';

// --- LIMITES DO BRASIL (Evita que o usuário arraste para fora) ---
const brasilBounds = [
  [-35.0, -75.0], // Ponto extremo Sudoeste
  [7.0, -34.0]    // Ponto extremo Nordeste
];

// --- CONTROLADOR DA CÂMERA (COM SUPORTE A RESET E FECHAR CARDS) ---
...
// --- PINOS FIXOS E LEVES (OTIMIZADOS PARA TOTEM - SEM LAG) ---
const createPin = (color) => {
  const htmlStr = `
    <div class="relative flex items-center justify-center w-8 h-8 group cursor-pointer">
      <div class="relative w-8 h-8 rounded-full border-[3px] border-white z-10 flex items-center justify-center" style="background-color: ${color}; box-shadow: 0 4px 8px rgba(0,0,0,0.3);">
...
export default function App() {
  ...
  // ESTADO DA LISTA: Controla se está expandida ou minimizada
  ...
  const resetTotem = useCallback(() => {
    ...
    setShowLogin(false);
    setShowModal(false);
    setIsListExpanded(true); // Expande a lista novamente quando o totem reseta
    setResetTrigger(prev => prev + 1); 
    showNotification("Totem Reiniciado", "Voltando à visão geral do Brasil.");
  }, []);
  ...
  useEffect(() => {
    ...
    const resetTimer = () => {
      ...
      timeoutId = setTimeout(resetTotem, 60000); // Reseta após 60 segundos sem tocar na tela
    };
  ...
  const handleAddPonto = (e) => {
    ...
    setMapZoom(16);
    setIsListExpanded(false); // Minimiza a lista ao adicionar um ponto novo
  };

  const handleLocationClick = (local) => {
    setMapCenter([local.lat, local.lng]);
    setMapZoom(16);
    setIsListExpanded(false); // A MÁGICA: Minimiza a lista automaticamente ao escolher um local!
  };
  ...
  const LogoBiscoite = () => (
    <svg width="180" height="65" viewBox="0 0 202 75" fill="none" xmlns="http://www.w3.org/2000/svg" className="drop-shadow-lg">
  ...
  return (
    <div className="relative w-full h-screen bg-b-dark overflow-hidden font-sans">
      
      {/* 1. MAPA DE FUNDO (COM LIMITES DO BRASIL) */}
      <div className="absolute inset-0 z-0">
        <MapContainer 
          center={[-15.79, -47.88]} 
          zoom={5} 
          minZoom={4} // Impede de tirar muito o zoom
          maxBounds={brasilBounds} // Trava dentro da "caixa" do Brasil
          maxBoundsViscosity={1.0} // Dá a firmeza de uma "parede" (1.0 = bloqueio total)
        ...
        >
          ...
          {locais.map(local => (
            <Marker key={local.id} position={[local.lat, local.lng]} icon={pinIcons[local.status] || pinIcons['Disponível']}>
              <Popup autoPan={false}>
                <div className="w-80 bg-b-dark rounded-3xl overflow-hidden shadow-2xl border border-b-primary/40 font-sans flex flex-col">
                  <div className="h-40 bg-gray-800 relative">
                    ...
                    <div className="absolute bottom-3 left-4 right-4 flex justify-between items-end">
                       <span className="text-[12px] font-bold px-3 py-1.5 rounded-lg uppercase shadow-lg text-white" style={{ backgroundColor: local.status === 'Alta Prioridade' ? '#E84393' : local.status === 'Em Negociação' ? '#E8B84B' : '#2ecc71' }}>
                         {local.status}
                       </span>
                    </div>
                  </div>
                  <div className="p-5">
                    <h3 className="font-bold text-xl text-b-light leading-tight mb-2">{local.nome}</h3>
                    <p className="text-sm text-b-secondary flex items-center gap-2 mb-5"><MapPin size={16}/> {local.cidade}</p>
                    <button onClick={() => window.open(`http://maps.google.com/?q=${local.lat},${local.lng}`, '_blank')} className="w-full py-4 bg-white/5 hover:bg-b-primary/20 text-b-secondary hover:text-white border border-b-primary/30 rounded-2xl text-sm font-bold transition-all flex items-center justify-center gap-2">
                      <ExternalLink size={18} /> ABRIR GPS
                    </button>
                  </div>
                </div>
              </Popup>
      ...
      {/* 2. HEADER FLUTUANTE */}
      <header className="absolute top-0 left-0 w-full z-40 bg-b-dark/80 backdrop-blur-xl border-b border-b-primary/30 p-6 flex justify-between items-center shadow-2xl">
        <div>
          <LogoBiscoite />
          <p className="text-b-secondary mt-2 font-bold text-lg tracking-wide">Plano de Expansão</p>
        </div>
        {isLoggedIn ? (
          <div className="flex gap-4">
            <button onClick={() => setShowModal(true)} className="bg-b-primary text-b-dark px-6 py-4 rounded-2xl font-bold text-lg flex items-center gap-3 shadow-lg shadow-b-primary/20 hover:bg-b-secondary transition-all">
              <Plus size={24} /> NOVO PONTO
            </button>
            <button onClick={() => setIsLoggedIn(false)} className="bg-white/5 text-b-secondary p-4 rounded-2xl border border-b-primary/30 hover:bg-red-500/10 hover:text-red-400 transition-all">
              <LogOut size={24} />
            </button>
          </div>
        ) : (
          <button onClick={() => setShowLogin(true)} className="text-b-secondary border-2 border-b-primary/30 px-6 py-4 rounded-2xl font-bold text-lg flex items-center gap-3 hover:bg-b-primary/10 hover:text-b-light transition-all bg-b-dark/50">
            <LogIn size={24} /> ACESSO RESTRITO
          </button>
        )}
      </header>

      {/* 3. CARD INFERIOR (MINIMIZAR/EXPANDIR) */}
      <div 
        className={`absolute bottom-8 left-1/2 -translate-x-1/2 w-[90%] max-w-[800px] bg-b-dark/90 backdrop-blur-2xl rounded-[2rem] shadow-[0_20px_60px_rgba(0,0,0,0.8)] border-2 border-b-primary/30 flex flex-col z-40 overflow-hidden transition-all duration-500 ease-in-out ${isListExpanded ? 'h-[45%]' : 'h-[140px]'}`}
      >
        <div 
          onClick={() => setIsListExpanded(!isListExpanded)}
          className="w-full flex justify-center items-center py-2 bg-white/5 cursor-pointer border-b border-b-primary/20 hover:bg-white/10 transition-colors"
        >
          {isListExpanded ? (
            <ChevronDown size={28} className="text-b-secondary" />
          ) : (
            <ChevronUp size={28} className="text-b-secondary" />
          )}
          <span className="text-b-secondary text-sm font-bold ml-2 uppercase tracking-widest">
            {isListExpanded ? 'Ocultar Lista' : 'Ver Todas as Lojas'}
          </span>
        </div>

        <div className="p-6 bg-white/5">
          <div className="relative">
            <Search className="absolute left-5 top-1/2 -translate-y-1/2 text-b-secondary/60" size={28} />
            <input 
              type="text" placeholder="Toque para buscar shopping..." 
              className="w-full pl-16 p-5 bg-b-dark border-2 border-b-primary/40 rounded-2xl text-xl text-b-light focus:outline-none focus:border-b-secondary transition-all shadow-inner"
              value={busca} 
              onChange={(e) => {
        ...
        <div className="overflow-y-auto flex-1 p-6 space-y-4 touch-pan-y custom-scrollbar border-t border-b-primary/20">
          {locais.filter(l => l.nome.toLowerCase().includes(busca.toLowerCase()) || l.cidade.toLowerCase().includes(busca.toLowerCase())).map(local => (
            <div 
              ...
              className="p-5 border-2 border-b-primary/20 rounded-2xl cursor-pointer bg-white/5 active:bg-b-primary/20 transition-all flex flex-col gap-2"
            >
              <div className="flex justify-between items-center mb-1">
                <span className="text-[12px] font-bold px-3 py-1.5 bg-b-dark text-b-secondary rounded-lg border border-b-primary/30">
                  {local.regiao}
                </span>
                <span className="text-[11px] font-bold px-3 py-1.5 rounded-lg uppercase tracking-wider" style={{
              ...
              <h4 className="font-bold text-b-light text-2xl">{local.nome}</h4>
              <p className="text-base text-b-secondary mt-1 flex items-center gap-2"><MapPin size={18}/> {local.cidade}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      {showLogin && !isLoggedIn && (
        <div className="fixed inset-0 flex items-center justify-center p-4 bg-b-dark/90 backdrop-blur-md z-50 overflow-hidden">
          <div className="bg-b-dark/90 p-8 rounded-3xl shadow-2xl w-full max-w-lg border border-b-primary/30">
            <div className="flex flex-col items-center mb-6">
              <LogoBiscoite />
              <p className="text-b-secondary mt-4 font-medium tracking-wide uppercase text-sm">Acesso Restrito - Expansão</p>
            </div>
            <form onSubmit={handleLogin} className="space-y-4">
              {loginError && <div className="bg-red-500/10 border border-red-500/50 p-4 rounded-xl flex items-center gap-2 text-red-400 text-sm"><AlertCircle size={20} /> {loginError}</div>}
              <div>
                <label className="text-sm font-bold text-b-secondary ml-1 mb-2 block uppercase">E-mail Corporativo</label>
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="ex: expansao@biscoite.com" className="w-full p-5 bg-white/5 border-2 border-b-primary/30 text-b-light rounded-xl focus:outline-none focus:border-b-secondary transition-all text-lg" required />
              </div>
              <div className="relative">
                <label className="text-sm font-bold text-b-secondary ml-1 mb-2 block uppercase">Senha</label>
                <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="w-full p-5 pr-14 bg-white/5 border-2 border-b-primary/30 text-b-light rounded-xl focus:outline-none focus:border-b-secondary transition-all text-lg" required />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-5 top-[44px] text-b-secondary hover:text-white transition-colors">
                  {showPassword ? <EyeOff size={24} /> : <Eye size={24} />}
                </button>
              </div>
              <button type="submit" className="w-full bg-b-primary text-b-dark p-5 rounded-xl font-bold hover:bg-b-secondary transition-all shadow-lg text-lg mt-4">ENTRAR NO PAINEL</button>
              <button type="button" onClick={() => setShowLogin(false)} className="w-full text-b-secondary text-sm font-medium hover:text-b-light transition-colors mt-2 p-3">Voltar ao mapa de visitante</button>
            </form>
          </div>
        </div>
      )}
      ...
      {/* 5. MODAL DE CADASTRO */}
      {showModal && (
        <div className="fixed inset-0 bg-b-dark/90 backdrop-blur-md z-[9999] flex items-center justify-center p-4">
          <div className="bg-b-dark w-full max-w-2xl rounded-3xl shadow-2xl border border-b-primary/30 overflow-hidden">
            <div className="bg-white/5 border-b border-b-primary/30 p-6 text-b-light flex justify-between items-center">
              <h3 className="text-2xl font-bold flex items-center gap-3 text-b-secondary"><MapPin /> Cadastrar Nova Praça</h3>
              <button onClick={() => setShowModal(false)} className="text-b-secondary hover:text-white transition-all"><X size={32}/></button>
            </div>
            <form onSubmit={handleAddPonto} className="p-8 grid grid-cols-2 gap-6">
              <div className="col-span-2">
                <label className="text-sm font-bold text-b-secondary uppercase tracking-wider mb-2 block">Nome do Local</label>
                <input required value={novoPonto.nome} onChange={e => setNovoPonto({...novoPonto, nome: e.target.value})} type="text" className="w-full p-4 bg-white/5 border border-b-primary/30 text-b-light rounded-xl focus:border-b-secondary" />
              </div>
              <div className="col-span-2">
                <label className="text-sm font-bold text-b-secondary uppercase tracking-wider mb-2 block">Cidade e UF</label>
                <input required value={novoPonto.cidade} onChange={e => setNovoPonto({...novoPonto, cidade: e.target.value})} type="text" className="w-full p-4 bg-white/5 border border-b-primary/30 text-b-light rounded-xl focus:border-b-secondary" />
              </div>
              <div className="col-span-2">
                <label className="text-sm font-bold text-b-secondary uppercase tracking-wider mb-2 block">Status Estratégico</label>
                <select value={novoPonto.status} onChange={e => setNovoPonto({...novoPonto, status: e.target.value})} className="w-full p-4 bg-b-dark border border-b-primary/30 text-b-light rounded-xl focus:border-b-secondary">
              ...
              <div className="col-span-2 grid grid-cols-2 gap-6">
                <div>
                  <label className="text-sm font-bold text-b-secondary uppercase tracking-wider mb-2 block">Latitude</label>
                  <input required value={novoPonto.lat} onChange={e => setNovoPonto({...novoPonto, lat: e.target.value})} type="text" className="w-full p-4 bg-white/5 border border-b-primary/30 text-b-light rounded-xl focus:border-b-secondary" />
                </div>
                <div>
                  <label className="text-sm font-bold text-b-secondary uppercase tracking-wider mb-2 block">Longitude</label>
                  <input required value={novoPonto.lng} onChange={e => setNovoPonto({...novoPonto, lng: e.target.value})} type="text" className="w-full p-4 bg-white/5 border border-b-primary/30 text-b-light rounded-xl focus:border-b-secondary" />
                </div>
              </div>
              <div className="col-span-2 pt-4">
                <button type="submit" className="w-full bg-b-primary text-b-dark p-5 rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-b-secondary transition-all shadow-lg text-lg">
                  <Save size={24} /> ADICIONAR AO MAPA
                </button>
              </div>
            </form>
          </div>
